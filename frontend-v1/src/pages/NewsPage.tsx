    ...
    <div className="flex flex-col gap-4 h-full min-h-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 items-center gap-4 shrink-0">
        ...
        <div className="flex justify-center">
          <div className="flex bg-background-card border border-border rounded-md p-1 gap-1">
            <button 
      ...
      <div className="flex-1 overflow-auto custom-scrollbar pr-2">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 pb-8">
          {filteredNews.length === 0 ? (
            <div className="col-span-full h-64 flex flex-col items-center justify-center text-text-muted opacity-50 italic border border-border rounded-lg bg-background-card">
            ...
            filteredNews.map((item) => (
              <div 
                key={item.id} 
                className={cn(
                  "bg-background-card border rounded-lg p-5 transition-all duration-300 relative overflow-hidden group flex flex-col gap-4",
                  item.is_relevant 
                    ? "border-accent-cyan/30 shadow-[0_0_20px_rgba(0,255,255,0.05)] ring-1 ring-accent-cyan/10" 
                    : "border-border hover:border-border5 hover:shadow-xl"
                )}
              >
                {item.is_relevant && (
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-accent-pink/30">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="p-2 bg-white/5 hover:bg-white/10 rounded-md text-text-secondary hover:text-white transition-all border border-border"
                    >
