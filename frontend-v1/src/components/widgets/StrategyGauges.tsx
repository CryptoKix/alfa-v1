...
const Gauge = ({ label, value, percentage, icon: Icon, color, glowColor }: GaugeProps) => {
  ...
  return (
    <div className="bg-background-card border border-accent-pink/30 rounded-lg p-4 shadow-floating relative overflow-hidden flex items-center justify-between group h-[100px]">
      {/* Top Accent Line */}
...
export const StrategyGauges = ({ onViewBots }: { onViewBots?: () => void }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr_1fr] gap-6 shrink-0">
      ...
      <button 
        onClick={onViewBots}
        className="bg-background-card border border-accent-pink/30 rounded-lg flex flex-col items-center justify-center gap-2 h-[100px] hover:bg-accent-cyan/5 hover:border-accent-cyan/20 transition-all group relative overflow-hidden shadow-floating"
      >
