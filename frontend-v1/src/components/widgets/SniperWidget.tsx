...
export const SniperWidget: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-background-card border border-accent-pink/30 rounded-lg overflow-hidden relative shadow-floating">
      
      {/* Header */}
      <div className="flex items-center justify-between mb-2 border-b border-accent-pink/30 shrink-0 h-[55px] -mx-4 px-4 -mt-4 p-3 bg-background-elevated/20">
      ...
      {/* Grid Header */}
      <div className="grid grid-cols-[1fr_100px_60px_80px_80px_90px] gap-2 px-3 pb-2 text-[9px] font-bold text-text-secondary uppercase tracking-wider shrink-0 mr-[6px]">
        ...
        {trackedTokens.length === 0 ? (
          ...
          trackedTokens.map((token) => (
            <div 
              ...
              className="grid grid-cols-[1fr_100px_60px_80px_80px_90px] gap-2 items-center px-3 py-2 rounded-lg bg-background-elevated/30 border border-border hover:bg-white/5 hover:border-border transition-colors group cursor-pointer"
            >
              {/* Asset */}
