...
export const ArbSettingsWidget = () => {
  ...
  return (
    <div className="lg:w-[400px] bg-background-card border border-accent-pink/30 rounded-lg p-4 shadow-floating relative overflow-hidden flex flex-col gap-4 h-full shrink-0">
      ...
      <div className="flex-1 bg-black/20 rounded-md border border-border overflow-hidden flex flex-col min-h-0 justify-center">
        <div className="flex-1 overflow-auto custom-scrollbar p-4 flex flex-col justify-center space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input type="number" value={minProfit} onChange={e => dispatch(setArbConfig({ minProfit: parseFloat(e.target.value) }))} className="w-full bg-black/40 border border-border rounded-md px-3 h-12 text-sm font-mono font-bold text-white focus:border-accent-cyan/50 outline-none transition-colors" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] uppercase text-text-muted font-bold px-1">Jito Tip (SOL)</label>
              <input type="number" value={jitoTip} onChange={e => dispatch(setArbConfig({ jitoTip: parseFloat(e.target.value) }))} className="w-full bg-black/40 border border-border rounded-md px-3 h-12 text-sm font-mono font-bold text-white focus:border-accent-cyan/50 outline-none transition-colors" />
            </div>
          </div>
      ...
      <button onClick={handleInitialize} disabled={status === 'loading'} className={cn("w-full py-4.5 rounded-lg text-black font-black text-sm uppercase tracking-[0.25em] transition-all flex items-center justify-center gap-3 shadow-floating", isMonitoring ? "bg-white/5 text-accent-purple border border-accent-cyan/30 shadow-none hover:bg-white/10" : "bg-accent-cyan shadow-glow-cyan hover:bg-white active:scale-95")}>
      ...
      <div className="flex items-center justify-between mb-1 border-b border-accent-pink/30 shrink-0 h-[55px] -mx-4 px-4 -mt-4">
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="p-1 bg-black/40 hover:bg-accent-cyan/20 rounded-md text-accent-purple border border-accent-cyan/50 hover:border-accent-cyan transition-all active:scale-95 ml-2"
          >
            <Plus size={14} strokeWidth={3} />
          </button>
        </div>
        <div className="text-[8px] font-mono text-accent-purple uppercase tracking-widest opacity-60 px-2">Live 5s Polling</div>
      </div>

      <div className="flex-1 bg-black/20 rounded-md border border-border overflow-hidden flex flex-col min-h-0 mt-2">
            ...
            {Object.entries(matrix).map(([pair, data]) => {
              ...
              return (
                <div key={pair} className="grid grid-cols-[85px_repeat(4,1fr)_32px] gap-1 h-10 items-stretch group">
                  <div className="bg-white/5 rounded-lg border border-border px-1.5 flex items-center min-w-0">
                  ...
                  {venues.map(v => {
                    ...
                    return (
                      <div key={v} className={cn("rounded-lg border px-1 flex flex-col items-center justify-center transition-all", isBest ? "bg-accent-purple/10 border-accent-cyan/30" : isWorst ? "bg-accent-purple/10 border-accent-pink/30" : "bg-white/[0.02] border-border")}>
                        <div className={cn("text-[9px] font-mono font-bold", isBest ? "text-accent-purple" : isWorst ? "text-accent-purple" : "text-white/60")}>
                  ...
                  <div className="flex items-center justify-center pr-2">
                    <button 
                      ...
                      className={cn(
                        "w-7 h-7 flex items-center justify-center rounded-lg transition-all border shadow-floating",
                        id 
                          ? "bg-accent-red/20 border-accent-red/50 text-accent-red hover:bg-accent-red hover:text-white shadow-accent-red/10" 
                          : "bg-white/5 border-border text-white/20 cursor-not-allowed"
                      )}
...
export const ArbOpportunitiesWidget = () => {
  ...
  return (
    <div className="flex-1 bg-background-card border border-accent-pink/30 rounded-lg p-4 shadow-floating relative overflow-hidden flex flex-col h-full shrink-0 min-w-0">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar p-3 space-y-2">
          {opportunities.length === 0 ? (
            ...
            opportunities.map((opp, i) => (
              <div key={i} className="p-3 rounded-md border border-border bg-white/[0.02] transition-all relative overflow-hidden group">
                <div className="flex items-start gap-3">
                  <div className="p-1.5 rounded-lg shrink-0 text-accent-purple bg-accent-purple/10"><Zap size={14} /></div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="text-[10px] font-black text-accent-purple">+{opp.spread_pct.toFixed(3)}%</div>
