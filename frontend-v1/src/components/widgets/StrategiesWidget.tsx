...
export const StrategiesWidget = ({ onSelect, selectedId, onViewBots, rightElement }: any) => {
  ...
  const terminalContent = (
    <div className={cn(
      "bg-background-card border border-accent-pink/30 rounded-lg p-4 shadow-floating relative overflow-hidden flex flex-col h-full shrink-0",
      ...
      <div className="flex items-center justify-between mb-1 border-b border-accent-pink/30 shrink-0 h-[55px] -mx-4 px-4 -mt-4">
        ...
        <button 
          onClick={onViewBots}
          className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white border border-border rounded-lg text-[8px] font-black uppercase tracking-wider transition-all transform active:scale-95 shrink-0"
        >
          Active Strategies
        </button>
      </div>
          ...
          {STRATEGIES.map((strat) => {
            ...
            return (
              ...
              >
                ...
                <div 
                  className={cn(
                    "absolute -inset-1 transition-all duration-500 blur-xl opacity-0 pointer-events-none rounded-md",
                ...
                <div className={cn(
                  "absolute inset-0 border transition-all duration-300 rounded-md flex flex-col items-center justify-center gap-1 overflow-hidden",
                  isSelected 
                    ? "bg-background-elevated border-accent-cyan shadow-[0_0_15px_rgba(0,255,255,0.1)]" 
                    : "bg-black/20 border-border group-hover/btn:border-border hover:bg-black/40",
                )}>
                  {/* Active Indicator */}
                  {isActive && (
